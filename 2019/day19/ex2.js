require('fs').readFile('./ex.input', (err, data) => {
    const program = data.toString().trim().split(',').map(x => parseInt(x));
    let map = new Map();
    const toVisit = new Set([`0,0`]);
    for (const point of toVisit) {
        const [x, y] = point.split(',').map(Number);
        if (checkOK(map, x, y, program)) {
            console.log(x, y, x * 10000 + y);
            break;
        }

        for (const [movX, movY] of [[1, 0], [0, 1]]) {
            const nextPos = `${x + movX},${y + movY}`;
            if (!toVisit.has(nextPos)) {
                toVisit.add(nextPos);
            }
        }
    }
...
function checkOK(map, x, y, program) {
    for (let addX = 0; addX < 100; addX++) {
        for (let addY = 0; addY < 100; addY++) {
            const val = getPoint(map, x + addX, y + addY, program);
            if (!val) {
                return false;
            }
        }
...
function getPoint(map, x, y, program) {
    if (map.has(`${x},${y}`)) {
        return map.get(`${x},${y}`)
    }
    const robot = new RobotModule(x, y);
    runProgram(program, robot, robot);
    map.set(`${x},${y}`, robot.output);
    return robot.output;
}
