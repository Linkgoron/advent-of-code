...
require('fs').readFile('./ex2.input', (err, data) => {
    const map = new Map(data.toString()
        .replace('@','$').replace('@','%')
        .replace('@','^').split('\n')
    ...
    const allPaths = new Map([...map].filter(([x, char]) => botSymbols.includes(char) || (char.charCodeAt(0) >= 97 && char.charCodeAt(0) <= 122))
        .map(([pos, char]) => [char, BFS(pos, map)]));

    const res = solver(botSymbols, allPaths);
    console.log(res);
});
...
function heuristic(state, map, allKeys) {
    return 0;
    const hasKey = allKeys.filter(key => botSymbols.includes(key) && state.keys.has(key));
    const missingKeys = allKeys.filter(key => botSymbols.includes(key) && !state.keys.has(key));
    ...
    return closestMissingKeys;
}

let hardBound = 5000;
let cnt = 0
function solver(initialState, maps) {
    const nxt = [{ keys: new Set(), botPos: initialState, steps: 0 }];
    const states = new Set(nxt);
    const travel = new Set(nxt);
    const uniqueKeySets = new Map();
    const allKeys = [...maps.keys()].filter(x => botSymbols.includes(x));
    for (const cur of travel) {
        cnt++;
        if (cnt % 1000 === 0) {
            console.log(cur);
        }
        const { botPos, keys, steps: curStep } = cur;
        if (curStep > hardBound) continue;
        states.delete(cur);
        if (keys.size === maps.size - 4) {
            console.log('viewed', cur);
            return curStep;
        }
        const computedKeys = new Set();

        for (const key of botPos) {
            ...
            for (const [nextKey, curM] of currentMap) {
                ...
                const nextBotPos = botPos.filter(x => x !== key);
                nextBotPos.push(nextKey);
                nextBotPos.sort((a, b) => a.charCodeAt(0) - b.charCodeAt(0));
                const uniqueString = [...keys]
                    .filter(x => !nextBotPos.includes(x))
                    .sort((a, b) => a.charCodeAt(0) - b.charCodeAt(0)).toString() + ',' + nextBotPos.toString();
                ...
                states.add({ botPos: nextBotPos, keys: nextKeys, steps: curStep + steps });
            }
        }
...
function BFS(starting, map) {
    ...
    for (const { pos, doors, steps, keysInPath } of toVisit) {
        ...
        if (curChar.charCodeAt(0) >= 65 && curChar.charCodeAt(0) <= 95 && !botSymbols.includes(curChar)) {
