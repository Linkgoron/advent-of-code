    ...
    for (const plan of plans) {
        ...
        for (let i = 0; i < 32; i++) {
            ...
            for (const origState of states.values()) {
                const expectedMax = origState.minerals.geode + origState.bots.geode * (32 - i);
                if (expectedMax > maybeMax) {
                    maybeMax = origState.minerals.geode + origState.bots.geode * (32 - i);
                }
                if (i > 15) {
                    if ((32 - i) * (Math.floor((32 - i) / 3) + origState.bots.geode) + origState.minerals.geode < maybeMax) {
                        continue;
                    }
                }
        ...
        values.push(maxGeode)
    }
    console.log(values.reduce((a,b) => a * b, 1));
});
