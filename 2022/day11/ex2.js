...
fs.promises.readFile('./ex.input').then(data => {
    ...
    const maxImportant = state.reduce((a,b) => a * b.divTest, 1);
    for (let round = 0; round < 10000  ; round++) {
        for (const [key, monkey] of monkeyMap) {
            for (const worry of monkey.items) {
                ...
                const canDiv = (afterOp % monkey.divTest) === 0;
                const targetMonkeyId = canDiv ? monkey.trueThrow : monkey.falseThrow;
                const target = monkeyMap.get(targetMonkeyId);
                target.items.push(afterOp % maxImportant);
            }
            monkey.prevInspectCount = monkey.inspectCount;
            monkey.inspectCount += monkey.items.length;
            monkey.diffs.push(monkey.inspectCount - monkey.prevInspectCount);
            monkey.items = [];
        }
    }
