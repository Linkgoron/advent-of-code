...
fs.promises.readFile(`${__dirname}${path.sep}ex.input`).then(data => {
    ...
    for (const scannerOne of scanners) {
        for (const scannerTwo of scanners) {
            const dist = Math.abs(scannerOne.location.x - scannerTwo.location.x) +
                Math.abs(scannerOne.location.y - scannerTwo.location.y) +
                Math.abs(scannerOne.location.z - scannerTwo.location.z);
            if (dist > maxDist) {
                maxDist = dist;
            }
        }
