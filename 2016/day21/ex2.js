...
fs.readFile('./ex.input', (err, data) => {
    ...
    const commands = data.toString().trim().split('\r\n').map(row => row.trim().split(' ')).map(parseCommand).reverse();
    let state = 'fbgdceah'
    for (const command of commands) {
        switch (command.command) {
            ...
            case 'rotateBased': {
                state = getPrecusor(state, command);
                continue;
            }
            case 'reverse': {
            ...
            case 'move': {
                const arr = [...state];
                const removed = arr.splice(command.to, 1);
                arr.splice(command.from, 0, removed[0]);
                state = arr.join('');
            }
        }
    }
...
function getPrecusor(originalState, command) {
    const arr = [...originalState];
    while (true) {
        const candidate = arr.join("");
        const res = rotateBased(command, candidate);
        if (res === originalState) {
            return candidate
        }
        arr.unshift(arr.pop());
    }
...
function rotateBased(command, state) {
