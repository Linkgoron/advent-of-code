...
fs.readFile('./ex.input', (err, data) => {
    ...
    function removeItems(floor, items) {
        return floor.filter(x => !items.includes(x)).sort((a, b) => b.type.localeCompare(a.type) || a.tech.localeCompare(b.tech));
    }
    ...
    function* nextStates(state) {
        ...
        const movementOptions = currentItems.map(x => [x]).concat([...allPairs(currentItems)]);
