...
fs.readFile('./ex.input', (err, data) => {
    ...
    console.log(results.sort((b, a) => b[0] - a[0])[0][0]);
    function* getBuilds() {
        for (const weapon of store.weapons) {
            ...
            for (let i = 0; i < store.rings; i++) {
                const firstRing = store.rings[i];
                yield [weapon.cost + firstRing.cost, weapon.damage + firstRing.damage, firstRing.armor];
                for (let j = i + 1; j < store.rings; j++) {
                    const secondRing = store.rings[j];
                    const curCost = weapon.cost + firstRing.cost + secondRing.cost;
                    const curDamage = weapon.damage + firstRing.damage + secondRing.damage;
                    const curArmor = firstRing.armor + secondRing.armor;
                    yield [curCost, curDamage, curArmor];
                }
            }
        }
