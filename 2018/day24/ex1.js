...
fs.readFile('./ex.input', (err, rawData) => {
    ...
    for (const row of rows) {
        ...
        currentTeam.push(parseGroupRow(row, currentTeamName, currentTeam.length))
    }
    ...
    console.log('immune state:', teams.immune.reduce((prev, cur) => {
        return prev + cur.units;
    }, 0));

    console.log('infection state:', teams.infection.reduce((prev, cur) => {
        return prev + cur.units;
    }, 0));
});
...
function parseGroupRow(row, currentTeamName, index) {
