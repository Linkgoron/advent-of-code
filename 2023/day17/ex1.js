...
fs.readFile('./ex.input', (err, data) => {
  ...
  while (heap.size() > 0) {
    ...
    if ((cur.x === board[0].length - 1) && (cur.y === board.length - 1)) {
      console.log(cur.heat);
      return;
    }
    for (const dir of Object.keys(dirs)) {
      if (cur.dir === dir && cur.sameDir === 3) continue;
      if (cur.sameDir !== 0 && opposites[cur.dir] === dir) continue;
      ...
      heap.add({ x: nextX, y: nextY, dir, heat: cur.heat + nextHeat, sameDir, path: [...cur.path, cur] });
    }
  }
});
