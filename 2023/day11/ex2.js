...
fs.readFile('./ex.input', (err, data) => {
  ...
  for (const galaxy of galaxies) {
    const belowExpanding = emptyRows.filter(r => galaxy.row > r).length * (expansionRate - 1);
    const rightOfExpanding = emptyCols.filter(c => galaxy.col > c).length * (expansionRate - 1);
