...
fs.readFile('./ex.input', (err, rawData) => {
    ...
    dummyNode.connected.first = { computeChain() { return 0; } };
    dummyNode.options = {
        first: [],
        second: components.filter(x => x.first === 0 || x.second === 0)
    }
    let res = buildChain(dummyNode, 0);
    console.log(res);
});
    ...
    return {
        ...
        computeChain(from) {
            ...
            if (this.connected.first && this.connected.first !== from) {
                res += this.connected.first.computeChain(this);
            }

            if (this.connected.second && this.connected.second !== from) {
                res += this.connected.second.computeChain(this);
            }
